...
with open(os.path.join(os.path.dirname(__file__), "data_mining_sharpener_parameters.json")) as f:
    parameters = json.load(f)
sentinel_2_reflectance = parameters["sentinel_2_reflectance"]
sentinel_3_lst = parameters["sentinel_3_lst"]
high_res_dem = parameters["high_res_dem"]
high_res_geom = parameters["high_res_geom"]
lst_quality_mask = parameters["lst_quality_mask"]
date_time_utc = datetime.datetime.strptime(parameters["date_time_utc"], "%Y-%m-%d %H:%M")
elevation_band = parameters["elevation_band"]
lst_good_quality_flags = parameters["lst_good_quality_flags"]
cv_homogeneity_threshold = parameters["cv_homogeneity_threshold"]
moving_window_size = parameters["moving_window_size"]
parallel_jobs = parameters["parallel_jobs"]
